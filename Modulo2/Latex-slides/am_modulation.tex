...
% Saída
\draw[->] (lpf.east) -- node[above, font=\small] {$\hat{m}(t)$} (6.5,0);
% Ponto de derivação após LPF
\coordinate (tap) at (5.5,0);
\fill (tap) circle (2pt);
% PLL abaixo, centrado entre mult e LPF
\node[bloco, fill=green!15, minimum width=2.5cm] (pll) at (2,-2.2) {PLL / Costas};
% Realimentação: desce da saída, vai ao PLL
\draw[->, green!60!black] (tap) -- (5.5,-2.2) -- (pll.east);
\node[font=\footnotesize, green!60!black] at (5.8,-1) {erro};
% PLL gera oscilador local: sai pela esquerda, sobe ao multiplicador
\draw[->, blue] (pll.west) -- (0.5,-2.2) -- (0.5,-0.55);
\node[font=\footnotesize, blue] at (-1,-1.5) {$2\cos(2\pi f_c t + \hat{\theta})$};
\end{tikzpicture}
\end{center}
...
\coordinate (corner) at (4.8,0);
\draw[->] (lf.east) -- (corner) -- (4.8,-1.6) -- (vco.east);
\draw[->] (vco.west) -- (0,-1.6) -- (pd.south);
\node[font=\tiny, blue] at (1.3,-1.85) {$2\cos(2\pi f_c t + \hat{\phi})$};
\draw[->] (corner) -- ++(1,0) node[right, font=\tiny] {$v_c(t)$};
\end{tikzpicture}
\end{center}
...
\only<1>{
\textbf{Detector de Fase (PD):} multiplica $r(t)$ pelo VCO e filtra

$e(t) = r(t) \times c_L(t) \xrightarrow{\text{LPF}} K_d \sin(\theta_e)$

Para erros pequenos ($\theta_e \ll 1$):
\[
\boxed{e(t) \approx K_d\, \theta_e(t)} \qquad \theta_e = \phi - \hat{\phi}
\]
}
...
\only<2>{
\textbf{No domínio de Laplace} (linearizado para $\theta_e$ pequeno):
...
\draw[->] (sum.east) -- node[above, font=\footnotesize] {$\Theta_e(s)$} (kd.west);
% F(s)
\node[bloco, fill=blue!15] (fs) at (4.5,0) {$F(s)$};
\draw[->] (kd.east) -- (fs.west);
% Kv/s
\node[bloco, fill=green!15] (kv) at (7,0) {$\dfrac{K_v}{s}$};
\draw[->] (fs.east) -- (kv.west);
% Saída
\draw[->] (kv.east) -- ++(1,0) node[right, font=\small] {$\hat{\Phi}(s)$};
% Realimentação
\draw[->] (8,0) |- (0,-1.5) -- (sum.south);
\end{tikzpicture}
\end{center}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXERCÍCIOS — AM DSB-SC
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Exercícios: AM DSB-SC}

% ============================================
\begin{frame}{Exercício DSB-SC 1: Espectro e Largura de Banda}
...
% ============================================
\begin{frame}{Exercício DSB-SC 2: Potência com Dois Tons}
...
% ============================================
\begin{frame}{Exercício DSB-SC 3: Erro de Sincronismo e PLL}
...
% ============================================
\begin{frame}{Exercício DSB-SC 4: Dimensionamento do PLL}
...
% ============================================
\begin{frame}{Exercício DSB-SC 5: Densidade Espectral de Potência}
