...
\begin{tikzpicture}[>=latex, scale=0.85]
% Multiplicador
\node[draw, circle, minimum size=0.9cm] (mult) at (0,0) {$\times$};
% Filtro passa-baixas
\node[draw, rectangle, minimum width=2.2cm, minimum height=0.7cm] (lpf) at (3,0) {Filtro PB};

% Entradas
\draw[->] (-2.5,0.5) -- node[above, font=\small] {$s(t)$} (mult.north west);
\draw[->] (-2.5,-0.5) -- node[below, font=\small] {$2\cos(2\pi f_c t)$} (mult.south west);

% Saídas
\draw[->] (mult.east) -- node[above, font=\small] {$v(t)$} (lpf.west);
\draw[->] (lpf.east) -- node[above, font=\small] {$y(t)$} (5,0);

% Oscilador local
\node[draw, rectangle, minimum width=1.5cm, minimum height=0.5cm] (osc) at (-2.5,-1.5) {Oscilador};
...
\begin{center}
\includegraphics[width=\figFull, trim=0 235bp 0 0, clip]{figures/cap4/am_conventional.pdf}
\end{center}

\textbf{$\mu = 0.5$:} Envelope sempre positivo, modulação parcial.

\textbf{$\mu = 1.0$:} Envelope toca zero -- modulação 100\%.
...
\textbf{Eficiência:} Fração que carrega informação
\[
\eta = \frac{P_{sb}}{P_s} = \frac{k_a^2 P_m}{1 + k_a^2 P_m}
\]

\textbf{Para tom único} ($P_m = 1/2$):
\[
