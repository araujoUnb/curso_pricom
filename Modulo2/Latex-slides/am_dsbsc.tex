...
\item \textbf{Degrau de fase} --- mudança brusca (ex: reflexão, chaveamento)
\item \textbf{Rampa de fase} --- desvio contínuo de frequência (ex: Doppler)
\end{enumerate}
}

\only<5>{
\textbf{Perturbação 1: Degrau de fase} (PLL 1ª ordem)
...
\textbf{Passo 3:} Voltar ao tempo (transformada inversa).
\[
\frac{\phi_0}{s + K} \quad\xrightarrow{\;\mathcal{L}^{-1}\;}\quad \boxed{\theta_e(t) = \phi_0\,e^{-Kt}, \quad t \geq 0}
\]

\textbf{Passo 4:} Valor em regime permanente ($t \to \infty$).
\[
...
\only<7>{
\textbf{Perturbação 2: Rampa de fase / Doppler} (PLL 1ª ordem)
...
\[
\theta_e(\infty) = \frac{\Delta\omega}{K}(1 - 0) = \boxed{\frac{\Delta\omega}{K}} \quad\text{(erro finito!)}
\]
}
...
O integrador no filtro $F(s)$ \textbf{elimina} o erro para rampa --- o PLL de 2ª ordem rastreia perfeitamente um desvio Doppler constante.
}
...
\[
\tau = \frac{1}{2\pi \times 2000} = \boxed{0{,}08\,\text{ms}} \quad\text{(rápido!)}
\]
...
Sim! PLL de 2ª ordem tem erro \textbf{zero} para rampa de fase:
\[
\theta_e(\infty) = 0 \quad\text{para Doppler constante}
\]

O de 1ª ordem sempre terá erro residual $\Delta\omega/K$, que só pode ser reduzido aumentando $K$ --- mas $K$ grande demais amplifica ruído.
}
