...
# Title: Exemplo 09 - Banda Basica vs Passante
# Author: Curso PriCom - UnB
# Description: Banda Basica vs Passante
...
class exemplo_09_banda_basica_passante(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Exemplo 09 - Banda Basica vs Passante", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Exemplo 09 - Banda Basica vs Passante")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "exemplo_09_banda_basica_passante")
        ...
        self.sig_lo = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, 5000, 2, 0, 0)
        self.sig_carrier = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, 5000, 1, 0, 0)
        self.sig_baseband = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, 300, 1, 0, 0)
        self.qtgui_time_sink = qtgui.time_sink_f(
            2048, #size
            samp_rate, #samp_rate
            "Tempo - Basica vs Passante vs Recuperado", #name
            3, #number of inputs
            None # parent
        )
        ...
        labels = ['Banda Basica (300Hz)', 'Banda Passante (em 5kHz)', 'Recuperado (apos LPF)', 'Signal 4', 'Signal 5',
        ...
        self.qtgui_freq_sink = qtgui.freq_sink_f(
            ...
            0, #fc
            samp_rate, #bw
            "Espectro - Basica vs Passante vs Recuperado", #name
            3,
            None # parent
        )
        ...
        labels = ['Banda Basica (300Hz)', 'Banda Passante (em 5kHz)', 'Recuperado (apos LPF)', '', '',
            '', '', '', '', '']
        widths = [2, 2, 2, 1, 1,
            1, 1, 1, 1, 1]
        colors = ["blue", "red", "green", "black", "cyan",
        ...
        self.blocks_multiply_up = blocks.multiply_vff(1)
        self.blocks_multiply_down = blocks.multiply_vff(1)


        ##################################################
        # Connections
        ##################################################
        self.connect((self.blocks_multiply_down, 0), (self.low_pass_filter, 0))
        self.connect((self.blocks_multiply_up, 0), (self.blocks_multiply_down, 0))
        self.connect((self.blocks_multiply_up, 0), (self.qtgui_freq_sink, 1))
        self.connect((self.blocks_multiply_up, 0), (self.qtgui_time_sink, 1))
        self.connect((self.blocks_throttle, 0), (self.blocks_multiply_up, 0))
        self.connect((self.blocks_throttle, 0), (self.qtgui_freq_sink, 0))
        self.connect((self.blocks_throttle, 0), (self.qtgui_time_sink, 0))
        self.connect((self.low_pass_filter, 0), (self.qtgui_freq_sink, 2))
        self.connect((self.low_pass_filter, 0), (self.qtgui_time_sink, 2))
        self.connect((self.sig_baseband, 0), (self.blocks_throttle, 0))
        self.connect((self.sig_carrier, 0), (self.blocks_multiply_up, 1))
        self.connect((self.sig_lo, 0), (self.blocks_multiply_down, 1))
    ...
    def set_samp_rate(self, samp_rate):
        ...
        self.sig_baseband.set_sampling_freq(self.samp_rate)
        self.sig_carrier.set_sampling_freq(self.samp_rate)
        self.sig_lo.set_sampling_freq(self.samp_rate)
        self.blocks_throttle.set_sample_rate(self.samp_rate)
        self.low_pass_filter.set_taps(firdes.low_pass(1, self.samp_rate, 1000, 200, window.WIN_HAMMING, 6.76))
...
def main(top_block_cls=exemplo_09_banda_basica_passante, options=None):
