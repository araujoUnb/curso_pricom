...
\begin{frame}{Visualização: Amostragem e Aliasing}

\begin{center}
\includegraphics[width=\figFull, height=0.75\textheight, keepaspectratio]{figures/cap3/sampling_aliasing}
\end{center}

Subamostragem ($f_s < 2f_{max}$) causa sobreposição espectral (aliasing).
...
\begin{frame}{Algoritmo FFT --- Decomposição}

\textbf{Decomposição butterfly (radix-2):}

\[
X[k] = \sum_{n=0}^{N-1} x[n] W_N^{kn}, \quad \text{onde } W_N = e^{-j2\pi/N}
\]
...
\begin{frame}{Algoritmo FFT --- Resultado}

Usando $W_N^2 = W_{N/2}$:

\[
X[k] = \underbrace{\sum_{r=0}^{N/2-1} x[2r] W_{N/2}^{kr}}_{X_{even}[k]} + W_N^k \underbrace{\sum_{r=0}^{N/2-1} x[2r+1] W_{N/2}^{kr}}_{X_{odd}[k]}
\]
...
Duas DFTs de tamanho $N/2$! Continua recursivamente até DFTs de tamanho 2 (triviais).

\end{frame}

% ============================================

\begin{frame}{Bibliotecas para FFT}

\textbf{Implementações disponíveis:}

\begin{itemize}
\item \textbf{Python/NumPy:} \texttt{numpy.fft.fft(x)} e \texttt{numpy.fft.fftfreq(N, d=Ts)}
\item \textbf{Python/SciPy:} \texttt{scipy.fft.fft(x)} --- mais opções e mais rápido
\item \textbf{MATLAB/Octave:} \texttt{fft(x)}, \texttt{fftshift(X)}
\item \textbf{C/C++:} FFTW --- \textit{Fastest Fourier Transform in the West}
\item \textbf{Hardware:} DSPs e FPGAs possuem blocos FFT dedicados
\end{itemize}

\vspace{0.4cm}

\textbf{Passos típicos em qualquer biblioteca:}

\begin{enumerate}
\item Coletar amostras $x[n]$ com taxa $f_s \geq 2f_{\max}$
\item Aplicar janela (Hanning, Hamming, etc.) se necessário
\item Calcular FFT com $N = 2^m$
\item Mapear índices $k$ para frequências $f_k = k\cdot f_s/N$
\item Usar $|X[k]|$ para espectro de magnitude
\end{enumerate}
...
\begin{frame}[fragile]{Exemplo: Código Python (NumPy)}
...
x = np.array([...])        # N amostras do sinal

# Janela e FFT
w = np.hanning(N)
X = np.fft.fft(x * w)

# Frequências e espectro de magnitude
freqs = np.fft.fftfreq(N, d=Ts)   # vetor de frequências
mag   = np.abs(X) / N              # magnitude normalizada
\end{verbatim}
...
\begin{frame}{Efeito das Janelas no Espectro}

\begin{center}
\includegraphics[width=\figFull, height=0.72\textheight, keepaspectratio]{figures/cap3/dft_windowing}
\end{center}

\vspace{-0.3cm}
\begin{block}{Compromisso fundamental}
Janela com menor vazamento (Blackman) \textbf{alarga} o lóbulo principal.
Janela retangular tem melhor resolução mas \textbf{pior} supressão de lóbulos laterais.
\end{block}
...
\begin{frame}{Espectrograma: Parâmetros e Aplicações}
