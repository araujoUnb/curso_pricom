...
\[
P = \frac{1}{2\pi} \cdot \frac{\pi A^2}{2} \cdot 2 = \frac{A^2}{2} \quad \checkmark
\]
...
\begin{frame}{Demonstração do Teorema (esboço --- 1/2)}
...
\begin{frame}{Demonstração do Teorema (esboço --- 2/2)}

Após manipulação e tomando limite $T \to \infty$:

\[
S(\omega) = \int_{-\infty}^{\infty} \underbrace{\left[\lim_{T \to \infty} \frac{1}{T} \int f(t) f^*(t-\tau) dt\right]}_{R(\tau)} e^{-j\omega\tau} d\tau
\]

\[
= \int_{-\infty}^{\infty} R(\tau) e^{-j\omega\tau} d\tau = \ft\{R(\tau)\}
\]

\begin{block}{Teorema de Wiener-Khinchin}
\[
S(\omega) = \ft\{R(\tau)\} \quad \blacksquare
\]
\end{block}
...
\begin{frame}{Exemplo: Autocorrelação de Senoide (1/2)}
...
\begin{frame}{Exemplo: Autocorrelação de Senoide (2/2)}
...
Confirma resultado obtido via coeficientes da série de Fourier!
...
\begin{center}
\includegraphics[width=\figFull, height=0.75\textheight, keepaspectratio]{figures/cap3/psd_noise}
\end{center}

Ruído branco tem PSD plana. Após filtragem, a PSD assume forma de $|H(\omega)|^2$.
...
\begin{frame}{Exemplo Prático: Link de Comunicação Wi-Fi}

\textbf{Parâmetros típicos Wi-Fi (802.11n):}

\begin{itemize}
\item Potência transmitida: $P_t = 20$ dBm (100 mW)
\item Banda: $B = 20$ MHz
\item Ruído térmico: $N_0/2$ com $N_0 = kT = 4 \times 10^{-21}$ W/Hz
\end{itemize}
