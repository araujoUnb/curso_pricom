...
A resposta ao impulso \textbf{caracteriza completamente} um sistema LTI!
...
$Y(\omega) = H(\omega) \cdot X(\omega)$, onde $H(\omega)$ é geralmente complexa:
\[
...
A resposta de um sistema LTI a uma entrada senoidal é também senoidal na mesma frequência. Se $x(t) = A\cos(\omega_0 t + \theta)$, então:
\vspace{-2mm}
\[
...
\textbf{Demonstração:}
$x(t) = \Real\{Ae^{j\theta}e^{j\omega_0 t}\}$.
...
\textbf{Na frequência:} Para estabilidade BIBO, $H(\omega)$ deve existir e ser limitada:
\vspace{-2mm}
\[
...
\begin{frame}{Função de Transferência Ideal -- Limitações}

\begin{block}{Condição de Transmissão sem Distorção}
\[
H(\omega) = K e^{-j\omega t_d} \quad \Rightarrow \quad |H(\omega)| = K, \quad \phi(\omega) = -\omega t_d
\]
\end{block}
...
%\textbf{Por quê?} Pelo critério de Paley-Wiener, a resposta em frequência de um sistema causal não pode ser zero em um intervalo contínuo de frequências. Um sistema com banda infinita e ganho perfeitamente constante é uma idealização irrealizável.

\vspace{0.3cm}

\textbf{Na prática:} busca-se aproximar a condição ideal na \textbf{banda de interesse} do sinal, aceitando distorção fora dessa banda.

\end{frame}

% ============================================

\begin{frame}{Canal Ideal vs Canal com Distorção}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/gnuradio_canal_ideal_vs_distorcido}
\end{center}
...
\begin{frame}{Distorção de Amplitude — Simulação}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/gnuradio_distorcao_amplitude}
\end{center}
...
\begin{frame}{Distorção de Fase — Simulação}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/gnuradio_distorcao_fase}
\end{center}
...
\begin{frame}{Distorção de Fase — Espectro de Magnitude e Fase}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/gnuradio_distorcao_fase_espectro}
\end{center}
...
\begin{frame}{Atraso de Grupo — Simulação}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/gnuradio_atraso_grupo}
\end{center}
...
\textbf{Largura de banda absoluta:}

Faixa onde $|H(\omega)| > 0$ (pode ser infinita).
\end{column}
\begin{column}{0.50\textwidth}
\includegraphics[width=\textwidth]{figures/cap3/largura_banda_3dB_abs}
\end{column}
\end{columns}
...
\begin{frame}{Largura de Banda de Sistemas — Banda de Ruído}

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Largura de banda de ruído:}
\[
B_n = \frac{1}{|H(\omega_{\max})|^2} \int_{-\infty}^{\infty} |H(\omega)|^2 d\omega
\]

Retângulo de mesma área que $|H(\omega)|^2$, com altura $|H(\omega_{\max})|^2$.
...
\end{column}
\begin{column}{0.50\textwidth}
\includegraphics[width=\textwidth]{figures/cap3/largura_banda_ruido}
\end{column}
\end{columns}
...
\begin{frame}{Largura de Banda na Prática}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/bandas_tecnologias}
\end{center}
...
\textbf{Transformada de Fourier} da equação $RC\,y'(t) + y(t) = x(t)$:
\[
Y(\omega)[1 + j\omega RC] = X(\omega) \quad \Rightarrow \quad H(\omega) = \frac{Y(\omega)}{X(\omega)} = \frac{1}{1 + j\omega RC}
\]
...
\begin{frame}{Exemplo 1: Magnitude e Fase do Filtro RC}
...
\begin{frame}{Visualização: Filtragem de Sinal por Sistema LTI}

\begin{center}
\includegraphics[width=\figFull, height=0.75\textheight, keepaspectratio]{figures/cap3/lti_system_response}
\end{center}

O filtro passa-baixas remove as componentes de alta frequência do espectro.
...
Reconhecemos: $\frac{1}{a + j\omega} \xleftrightarrow{\ft} e^{-at}u(t)$. Portanto:
\[
...
\begin{frame}{Exemplo Prático: Equalização de Canal de Áudio}

\textbf{Problema:} Um cabo de áudio de 50 m atenua altas frequências.

Modelo do cabo: $H_c(\omega) = \frac{1}{1 + j\omega/\omega_c}$ com $f_c = 5$ kHz

\textbf{Efeito:} Sons agudos ($f > 5$ kHz) são atenuados $\Rightarrow$ áudio ``abafado''

\textbf{Solução --- Equalizador:}
\[
H_{eq}(\omega) = 1 + j\omega/\omega_c \quad \Rightarrow \quad H_{eq} \cdot H_c = 1
\]

Na prática: um filtro que \textbf{amplifica} altas frequências para compensar.

É exatamente o que um equalizador de áudio faz: ajusta ganhos em diferentes faixas de frequência para compensar as características do canal!

\end{frame}

% ============================================

\begin{frame}{Efeito do Cabo no Sinal}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/equalizacao_audio_cabo}
\end{center}
...
\begin{frame}{Equalizador: Compensando o Cabo}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/equalizacao_audio_equalizador}
\end{center}
...
\begin{frame}{Resultado: Sinal Restaurado}

\begin{center}
\includegraphics[width=\figFull, height=0.85\textheight, keepaspectratio]{figures/cap3/equalizacao_audio_tempo}
\end{center}
