...
\newcommand{\drawpipeline}[1]{%
  ...
  \begin{tikzpicture}[
    ...
    % ---------- RÓTULOS DOS BLOCOS ----------
    \node[grplbl, above=3pt] at (3.3, 0.34)  {Conversão A/D};
    \node[grplbl, below=3pt] at (3.3, -1.74) {Transmissão Digital};
  \end{tikzpicture}
  \end{center}%
}
...
\begin{frame}{Visão Geral: Da Amostragem à Modulação}
  ...
  \vspace{0.15cm}
  {\footnotesize
  Estudaremos cada etapa desta cadeia: a \textbf{conversão A/D}
  (filtro anti-aliasing, amostragem, quantização e PCM) e a
  \textbf{transmissão digital} em banda base (codificação de linha,
  formatação de pulso, diagrama de olho e PAM).}
\end{frame}
