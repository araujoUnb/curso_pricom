...
# -*- coding: utf-8 -*-
"""
Prática 06 – Phase-Locked Loop (PLL): Gabarito do Professor
=============================================================
Executa o flowgraph da Prática 06 em modo headless (sem GUI),
...
Execução:
    /usr/bin/python3 pratica_06_gabarito.py
...
try:
    ...
    print("Execute com: /usr/bin/python3 pratica_06_gabarito.py")
    sys.exit(1)
...
def etapa4():
    ...
    cap_theoretical = loop_bw * SAMP_RATE / (2 * np.pi)

    log(f"\n  Faixa de captura medida: {cap_min:.0f} — {cap_max:.0f} Hz")
    log(f"  Largura medida: {cap_max - cap_min:.0f} Hz")
    log(f"  Valor teórico aprox. (loop_bw * fs / 2π): "
        f"±{cap_theoretical:.1f} Hz ao redor de 0 Hz")
    ...
    log(f"    ≈ 2 × loop_bw × samp_rate / (2π) em Hz.")
    log(f"    Com loop_bw={loop_bw}, o valor teórico é")
    log(f"    ≈ {2*cap_theoretical:.1f} Hz. O valor medido deve ser")
    log(f"    próximo (limitado por max_freq/min_freq do bloco).")
...
if __name__ == '__main__':
    log("=" * 65)
    log("PRÁTICA 06 — PHASE-LOCKED LOOP (PLL)")
